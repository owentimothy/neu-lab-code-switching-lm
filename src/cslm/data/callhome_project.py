...
* CALLHOME rows are **never** eligible for ``CsCont`` (Bangor-sourced only).
...
# Conditions a *clean* CALLHOME row of each source may feed. CALLHOME is the
# monolingual source only: English -> EnglishMono + English side of MonoCont;
# Spanish -> SpanishMono + Spanish side of MonoCont. CsCont is never present
# here (it is Bangor-sourced only).
_SOURCE_CLEAN_CONDITIONS: dict[str, list[str]] = {
    "callhome_eng": ["EnglishMono", "MonoCont"],
    "callhome_spa": ["SpanishMono", "MonoCont"],
}

# Guard the sourcing invariant at import time: CALLHOME must never route to CsCont.
assert all(
    "CsCont" not in conditions for conditions in _SOURCE_CLEAN_CONDITIONS.values()
), "CALLHOME rows must never be eligible for CsCont"
...
def callhome_condition_candidates(source: str, screening_outcome: str) -> list[str]:
    """Condition candidates for a CALLHOME row, honoring source + screening.

    Only ``clean`` rows are eligible, and only for the monolingual conditions of
    their source language. Non-clean rows return ``[]`` (never silently admitted).
    ``CsCont`` is never returned.
    """
...
class CallhomeProjectedRow:
    ...
    def __post_init__(self) -> None:
        if self.source not in _SOURCE_CLEAN_CONDITIONS:
            raise ValueError(f"unknown CALLHOME source: {self.source!r}")
        if self.screening_outcome not in SCREENING_OUTCOMES:
            raise ValueError(f"unknown screening_outcome: {self.screening_outcome!r}")
        # Defense in depth: a CALLHOME row can never carry CsCont.
        if "CsCont" in self.condition_candidates:
            raise ValueError("CALLHOME rows must never be eligible for CsCont")
