...
# CALLHOME feeds the monolingual conditions only; CsCont is Bangor-sourced and
# must never appear on a CALLHOME row (enforced by _check_row_invariants).
...
def _check_row_invariants(rows: list[CallhomeProjectedRow]) -> None:
    ...
    known, and CsCont / non-monolingual condition candidates are never allowed.
    """
    for row in rows:
        if row.source not in CALLHOME_SOURCES:
            raise ValueError(f"unknown CALLHOME source: {row.source!r}")
        if row.screening_outcome not in SCREENING_OUTCOMES:
            raise ValueError(f"unknown screening_outcome: {row.screening_outcome!r}")
        if "CsCont" in row.condition_candidates:
            raise ValueError(
                "CALLHOME row must never include CsCont in condition_candidates"
            )
        unknown = set(row.condition_candidates) - set(MONOLINGUAL_CONDITIONS)
        if unknown:
            raise ValueError(
                f"CALLHOME row has non-monolingual condition candidates: {sorted(unknown)}"
            )
...
def summarize_projected_rows(
    ...
    for row in rows:
        condition_counts.update(row.condition_candidates)
