...
def test_diagnostics_rejects_validated_decision_with_extra_positive_reason():
    bad = explicit_source_validation("eng")
    bad.reason_codes = ["explicit_source_validation", "lexicon_expected_only"]
    with pytest.raises(ValueError):
        summarize_source_validation_decisions([bad])


def test_diagnostics_rejects_default_decision_with_duplicate_reasons():
    bad = default_source_validation("eng")
    bad.reason_codes = ["not_validated", "not_validated"]
    with pytest.raises(ValueError):
        summarize_source_validation_decisions([bad])
